...
function fetchGamesForWeek(week) {
    ...
    fetch(`/api/games?week=${week}`)
        ...
        .catch(err => {
            gamesList.innerHTML = '<div class="col-span-full text-center py-8"><p class="text-red-500">Error loading games.</p></div>';
            console.error(err);
        });
}
...
async function renderPicksTableWithOptions() {
    ...
    const picks = await fetchPicksForWeek(week);

    // If in results mode, fetch results
    let results = {};
    if (!picksMode) {
        const res = await fetch(`/api/results?week=${week}`);
    ...
    PLAYERS.forEach((player, playerIndex) => {
        ...
        CATEGORIES.forEach(cat => {
            ...
            if (!picksMode) {
                // Results mode: show color-coded outcome
                const outcome = results[player]?.[cat]?.outcome || 'pending';
                const pickValue = results[player]?.[cat]?.pick || currentPick || '';
                let outcomeClass = '';
                if (outcome === 'win') outcomeClass = 'bg-green-100 border-green-500 text-green-800 font-bold';
                else if (outcome === 'loss') outcomeClass = 'bg-red-100 border-red-500 text-red-800 font-bold';
                else if (outcome === 'tie') outcomeClass = 'bg-yellow-100 border-yellow-500 text-yellow-800 font-bold';
                else outcomeClass = 'bg-gray-100 border-gray-300 text-gray-500';

                html += `<div class="px-3 py-2 rounded border-2 ${outcomeClass}">${pickValue || '<span class="text-gray-400">No Pick</span>'}<br><span class="text-xs uppercase">${outcome}</span></div>`;
            } else if (currentWeekLocked) {
                // Locked: show static pick
                if (currentPick) {
                    html += `<div class="px-3 py-2 bg-gray-100 rounded border text-sm font-medium">${currentPick}</div>`;
                } else {
                    html += `<div class="px-3 py-2 bg-gray-50 rounded border text-sm text-gray-500">No Pick</div>`;
                }
            } else {
                // Editable dropdown
